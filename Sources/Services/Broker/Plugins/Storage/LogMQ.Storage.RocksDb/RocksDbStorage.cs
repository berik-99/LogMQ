...
public class RocksDbStorage : ILogStorage, IDisposable
{
	...
	/// <inheritdoc />
	public async Task<List<LogMessage>> GetLogsByFilterAsync(LogFilter filter)
	{
		...
		try
		{
			...
			foreach (var columnFamily in columnFamilies)
			{
				...
				while (iterator.Valid() && logMessages.Count < filter.Count)
				{
					...
					if (timestamp >= filter.TimeTo && timestamp <= filter.TimeTo)
					{
			...
			return logMessages.OrderBy(msg => msg.Timestamp).Take(filter.Count).ToList();
		}
		finally
		{
			semaphoreSlim.Release();
