...
public class PluginManager : IPluginManager
{
	...
	public async Task<PluginConfig> InstallPluginAsync(string pluginPath)
	{
	...
	public async Task<List<PluginConfig>> ListPluginsAsync(PluginConfigType configType, List<PluginStatus> statusFilter = null, PluginType? type = null)
	{
	...
	public async Task<PluginConfig> GetPluginConfigAsync(PluginConfigType configType, Guid pluginId)
	{
		var configFile = configType == PluginConfigType.Running ? PluginConfigBackupFile : PluginConfigFile;
		await using FileStream fileStream = new(configFile, FileMode.Open, FileAccess.ReadWrite, FileShare.None);
		List<PluginConfig> plugins = await LoadPluginsConfigAsync(fileStream);
	...
	public async Task<PluginConfig> GetPluginInfoAsync(PluginConfigType configType, string pluginPath)
	{
		if (!Path.Exists(pluginPath))
			throw new FileNotFoundException($"Plugin {pluginPath} not found");
		await using var file = File.OpenRead(pluginPath);
