...
internal static class LogEventExtensions
{
    ...
    /// (e.g., added by <see cref="MetadataEnricher"/>) or an exception message.
    /// </param>
    /// <returns>
    ...
    internal static LogMetadata ExtractLogMetadata(this LogEvent logEvent)
    {
        ...
        int row = 0;
        bool allPropertiesExtracted = logEvent.TryGetProperty(nameof(LogMetadata.File), out string file) &&
                                      logEvent.TryGetProperty(nameof(LogMetadata.Class), out className) &&
                                      logEvent.TryGetProperty(nameof(LogMetadata.MethodName), out methodName) &&
                                      logEvent.TryGetProperty(nameof(LogMetadata.MethodSignature), out methodSignature) &&
                                      logEvent.TryGetProperty(nameof(LogMetadata.Row), out row);
    ...
    internal static Guid ExtractLogId(this LogEvent logEvent)
    {
        logEvent.TryGetProperty(nameof(Guid), out Guid guid);
        return guid;
    }

    // Helper method to extract a property of a given type from the LogEvent.
    private static bool TryGetProperty<T>(this LogEvent logEvent, string propertyName, out T value)
    {
        value = default;
